...
app.get('/get-nonce', async (req, res) => {
    ...
    try {
        ...
        if (manualProxy && manualProxy.startsWith("0x") && manualProxy.length === 42) {
             ...
             const nonce = await client.readContract({ address: manualProxy, abi: SAFE_ABI, functionName: 'nonce' });
             return res.json({ nonce: nonce.toString(), proxy: manualProxy.toLowerCase() });
        }
...
app.get('/portfolio', async (req, res) => {
    ...
    if (!proxy) proxy = await resolveProxy(userLower);
