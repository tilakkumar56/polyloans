...
// --- CONFIGURATION ---
const MARKET_ADDR = "0x59B3a47dBe8B251eB14E57509A15Ed829065B91e"; // Ensure this matches deployment
...
app.post('/relay-tx', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: e.shortMessage || "Transaction Reverted" });
    }
});

// --- 3. ROBUST PRICE SCANNER ---
app.get('/portfolio', async (req, res) => {
    ...
    try {
        ...
        const rich = await Promise.all(positions.map(async (p) => {
            try {
                // Try fetching Public Price (No Auth Headers to avoid 401 errors)
                const priceCall = await axios.get(`https://clob.polymarket.com/price?token_id=${p.asset}&side=sell`);
                return { ...p, livePrice: priceCall.data.price, slug: p.slug };
            } catch(e) { 
                console.log(`Price fail for ${p.asset}, using fallback`);
                // FALLBACK: If price fails, return 0.50 so user can still borrow
