...
// --- 🔥 AUTOMATIC PROXY RESOLVER (FIXED) 🔥 ---
async function resolveProxy(user) {
    if(!user) return null;
    
    // 1. Checksum the address (Fixes the 422 Error)
    let cleanAddr;
    try {
        cleanAddr = getAddress(user); // Converts to 0xF18e... format
    } catch(e) { return null; }

    console.log(`🕵️‍♂️ Resolving Proxy for: ${cleanAddr}`);

    // Strategy 1: Gnosis Safe Registry (Now works because address is Checksummed)
    try {
        const r = await axios.get(`https://safe-transaction-polygon.safe.global/api/v1/owners/${cleanAddr}/safes/`);
    ...
    } catch(e) { console.log(`   Safe Registry: ${e.response?.status || e.message}`); }

    // Strategy 2: Gamma API
    try {
        const r = await axios.get(`https://gamma-api.polymarket.com/users/${cleanAddr.toLowerCase()}`);
    ...
    console.log("❌ No Proxy Found automatically.");
    return null;
}

// --- ENDPOINTS ---
app.get('/', (req, res) => res.send('Auto-Scanner Online'));
