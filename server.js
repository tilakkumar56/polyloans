...
// --- PROXY MAP ---
// Add any known proxies here to speed up lookup
const PROXY_MAP = { 
    ...
    try {
        ...
        const rich = await Promise.all(valid.map(async (p) => {
            try {
                ...
                // Fallback if price API fails
                return { ...p, livePrice: "0.50", slug: p.slug || "unknown" }; 
            }
        }));
        res.json(rich);
    } catch (e) { res.json([]); }
...
async function sendSafeTx(safeAddr, to, data, userSignature) {
    ...
    // Force 500k gas to ensure execution
